...
topo.addEventListener(
    "click",
    function() { document.getElementById("body").className = "body topo"; });

/* topography selected */
wave.addEventListener(
    "click",
    function() { document.getElementById("body").className = "body wave"; });

/* topography selected */
graph.addEventListener(
    "click",
    function() { document.getElementById("body").className = "body graph"; });
...
function RGB_Linear_Shade(p, color) {
  var i = parseInt, r = Math.round, [ a, b, c, d ] = color.split(","),
      P1 = p < 0, t = P1 ? 0 : 255 * p, P = P1 ? 1 + p : 1 - p;
  return ("rgb" + (d ? "a(" : "(") +
