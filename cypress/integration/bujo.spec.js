...
context("Actions", () => {
  ...
  it("Test signup", () => {
    ...
    cy.get("#signup-error")
        .should(($err) => {expect($err).to.contain('already in use')});
  ...
  it("Test password reset", () => {
    ...
    cy.wait(1000);
    cy.get(".btn").click();
    cy.get("#reset-error").should(($err) => {expect($err).to.contain('badly')});
    cy.get("#email")
        .clear()
        .type("test@sdkj.com")
    ...
    cy.get(".btn").click();
    cy.get("#reset-error")
        .should(($err) => {expect($err).to.contain('no user record')});
    cy.get("#email")
        .clear()
        .type("test@test.com")
        .should("have.value", "test@test.com");
    cy.get(".btn").click();
    cy.get("#reset-error")
        .should(($err) => {expect($err).to.contain('password reset link')});
  });
